...
def paint_perimeter():
  ...
  post: The perimeter is filled with beepers
  '''
  while front_is_clear() and corner_color_is(""):
    paint_corner('Green')
    move()
    if front_is_blocked():
      turn_left()
  paint_perimeter('Green')
...
def color_staircase():
  ...
  put_color_line() # handles fencepost error
...
def inner_layers():
  while corner_color_is(""):
    color = get_color()
    for _ in range(4): # Color the layer corners
      while corner_color_is(""):
        paint_corner(color)
        move()
        if not corner_color_is(""):
          step_back()
          turn_left()
      if front_is_clear():
        move()
...
# if __name__ == "__main__":
#   run_karel_program("12x16withbeepers")
...
def garbage_karel():
  while left_is_clear():
    while front_is_clear():
      while beepers_present(): # handle multiple beepers
        pick_beeper()
      move()
    while beepers_present(): # handle multiple beepers
        pick_beeper()
